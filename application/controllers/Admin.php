...
class Admin extends CI_Controller {
	...
	function laporan()
	{
		if ($this->uri->segment(3) == 'cetak') {
			if ($this->input->post('bulan') != '' and $this->input->post('bulan') != null and is_numeric($this->input->post('bulan')) and $this->input->post('tahun') != '' and $this->input->post('tahun') != null and is_numeric($this->input->post
			('tahun'))) {
				// echo "sini";
				$cek_tahun = $this->madmin->tanggal_terakhir_bulan($this->input->post('bulan'),$this->input->post('tahun'));

				$cek_data = $this->madmin->tampil_data_where('tb_pembelian',"(id_transaksi != 1 and id_transaksi != 2) and waktu_pembelian BETWEEN '".
				$this->input->post('tahun')."-".$this->input->post('bulan')."-01 00:00:00' AND '".$cek_tahun." 23:59:59' ORDER by waktu_pembelian DESC");
				// print_r(count($cek_data->result()));
